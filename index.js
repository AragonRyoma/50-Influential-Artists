...
function removeArtist(Arr, numie) {
  Arr.shift(numie);
}
...
function lotsOfArt(Arr) {
  ...
  for (let i = 0; i < Arr.length; i++)
    if (Arr[i].paintings > 100) {
      beastPainters.push(Arr[i].name);
    }

  return beastPainters;
}
...
function addArtist() {
  let Jean = {
    ...
    bio:
      "Jean Taylor was born as John Alexander Garcia Torres and has done nothing but ruin my life. I still love him. Barely.",
  };
